        ...
        <>
            {list.map((item, index) => {
                return <Category_item key={index} onClick={()=>handleClick(item.id)}>
                    <Image src={item.thumbnailImage} alt='item' width='1000' height='1000'/>
                    <h6 className='title'>{item.title}</h6>
                    <s className='price1'>{formatNumber(item.totalPrice)}</s>
                    <div className='price2'>{formatNumber(item.finalPrice)}</div>
